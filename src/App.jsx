...
// Render Backend Adresin (Sonunda / yok)
...
const GlobalStyles = () => (
  <style>{`
    ...
    .typing-cursor::after { content: '|'; animation: cursor-blink 1s step-end infinite; color: #22d3ee; }
...
const TechBackground = () => (
  <div className="fixed inset-0 z-0 pointer-events-none bg-[#020617] overflow-hidden">
    ...
    <div className="absolute top-[40%] right-[15%] text-cyan-500/30 font-mono text-xs animate-[blink-slow_5s_infinite_1s]"><div className="text-2xl font-thin">+</div><span className="block mt-1">Z-Depth: Active</span></div>
    <div className="absolute bottom-[20%] left-[8%] text-cyan-500/30 font-mono text-xs animate-[blink-slow_6s_infinite_0.5s]"><div className="text-2xl font-thin">+</div></div>
    <div className="absolute bottom-[10%] right-[5%] text-cyan-500/40 font-mono text-xs flex items-center gap-2 animate-[blink-slow_7s_infinite]"><span>SCALE: 1:1</span><div className="w-4 h-4 border-r border-b border-cyan-500/60 relative"><div className="absolute -bottom-[1px] -right-[1px] w-1 h-1 bg-cyan-400"></div></div></div>
    ...
    return (
      <div className="w-full max-w-4xl mx-auto mt-16 bg-slate-900/90 rounded-xl border border-slate-600 overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] relative group backdrop-blur-sm transform transition-transform hover:scale-[1.01] duration-500">
        ...
        <div className="p-8 md:p-12 min-h-[400px] flex items-center justify-center relative bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] transition-all duration-500">
          ...
          {activeTab === 'before' ? (
            <div className="relative z-10 animate-fade-in text-center text-red-400 font-mono font-bold bg-slate-950/90 px-6 py-4 rounded border border-red-900/50 backdrop-blur-md shadow-2xl flex flex-col gap-2"><span>❌ Pixelated</span><span>❌ Unsafe Lines</span></div>
          ) : (
             <div className="relative z-10 text-center text-cyan-400 font-mono font-bold bg-slate-950/90 px-6 py-4 rounded border border-cyan-900/50 backdrop-blur-md shadow-2xl flex flex-col gap-2"><span>✓ Vector Ready</span><span>✓ Cut Safe</span></div>
          )}
        </div>
      </div>
    );
...
export default function App() {
  ...
  const handleBuy = async (packageType) => {
    ...
    const email = prompt("Enter email for License Key receipt:");
    if(!email) return;
  ...
  const logout = () => { localStorage.removeItem('cuttora_key'); setApiKey(''); setResult(null); };

  // --- STATIC DATA ---
  const brands = ['LaserCo', 'MetalArt', 'CricutPro', 'EtsyMakers', 'CNCMasters', 'FabLab'];
  const doubledBrands = [...brands, ...brands]; 
  const testimonials = [
    { quote: "Finally a tool that closes open paths automatically. My plasma cutter didn't stop once during a 4-hour job.", author: "Mike T.", role: "CNC Plasma Operator" },
    ...
    { quote: "Reduced my prep time from 30 mins to 30 seconds.", author: "Emily W.", role: "Etsy Seller" },
    { quote: "Best vectorizer for metal art. It respects the minimum kerf width.", author: "Robert P.", role: "Sign Maker" }
  ];
  ...
  const faqs = [
    ...
    { q: "When will Smart Auto-Bridging be available?", a: "We are currently testing the Smart Auto-Bridging feature with our private beta group." },
  ...
  return (
    <div className="min-h-screen text-slate-100 font-sans selection:bg-cyan-500/30 selection:text-cyan-200 overflow-x-hidden relative">
      ...
      {apiKey ? (
        ...
        <section className="pt-44 pb-24 px-6 min-h-screen">
                             ...
                             <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-6 animate-fade-in">
                                 ...
                                 <div className="grid md:grid-cols-2 gap-4">
                                     ...
                                     <div className="flex flex-col justify-center gap-4">
                                         <a href={`${API_URL}${result.files.zip}`} className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 px-6 rounded-lg flex items-center justify-center gap-2 transition-colors"><Download className="w-5 h-5"/> Download ZIP Bundle</a>
                                         <a href={`${API_URL}${result.files.svg}`} target="_blank" className="w-full border border-slate-600 hover:border-white text-slate-300 hover:text-white font-bold py-3 px-6 rounded-lg flex items-center justify-center gap-2 transition-colors">View SVG</a>
                                     </div>
                                 </div>
                             </div>
                         )}
        ...
        // --- LANDING PAGE (FULL ORIGINAL DESIGN) ---
        <>
            ...
            <section id="pricing" className="py-32 relative z-10">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto items-stretch">
                    <Reveal delay={100} className="h-full"><PricingCard plan="Starter" description="5 Credits" features={["5 High-Res Conversions", "Standard SVG export", "Basic Optimization"]} onJoin={() => handleBuy('starter')} /></Reveal>
                    <Reveal delay={200} className="h-full"><PricingCard plan="Pro" description="50 Credits" recommended={true} features={["50 High-Res Conversions", "Advanced DXF & SVG", "Priority Processing", "Node Reduction"]} onJoin={() => handleBuy('pro')} /></Reveal>
                    <Reveal delay={300} className="h-full"><PricingCard plan="Agency" description="150 Credits" features={["150 High-Res Conversions", "Lowest Cost Per File", "Commercial License", "Bulk Processing Tools"]} onJoin={() => handleBuy('agency')} /></Reveal>
                    <Reveal delay={400} className="h-full"><PricingCard plan="Enterprise" description="Custom Solutions" features={["Unlimited API Access", "Custom Integration", "Dedicated Support"]} onJoin={() => handleBuy('Enterprise')} /></Reveal>
                </div>
                </div>
            </section>
            ...
            <section id="faq" className="py-32 relative z-10">
                ...
                <div className="space-y-4">
                    {faqs.map((item, i) => (<Reveal key={i} delay={i * 100}><FAQItem question={item.q} answer={item.a} /></Reveal>))}
                </div>
                </div>
            </section>
      ...
      <footer className="bg-slate-950 border-t border-slate-800 py-16 relative z-10 text-base font-medium">
        <div className="max-w-7xl mx-auto px-6 text-center text-slate-400">
            <div className="grid md:grid-cols-4 gap-12 mb-16 text-left">
                <div><h4 className="font-bold text-white mb-6 text-xl">Product</h4><ul className="space-y-4 text-slate-300"><li>Features</li><li>Pricing</li></ul></div>
                <div><h4 className="font-bold text-white mb-6 text-xl">Legal</h4><ul className="space-y-4 text-slate-300"><li>Privacy</li><li>Terms</li></ul></div>
            </div>
            <p>© 2025 Cuttora Inc. Built for Makers.</p>
        </div>
      </footer>
    </div>
  );
