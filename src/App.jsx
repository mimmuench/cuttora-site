...
export default function App() {
  ...
  return (
    <div className="min-h-screen text-slate-100 font-sans selection:bg-cyan-500/30 selection:text-cyan-200 overflow-x-hidden relative">
      ...
      {apiKey ? (
        <section className="pt-8 pb-24 px-6 min-h-screen relative z-10 bg-[#020617]">
          <div className="max-w-6xl mx-auto space-y-6">
            
            {/* 1. ÜST KATMAN: DEV ÖNİZLEME PANELİ & KREDİ YÖNETİMİ */}
              ...
              <div className="grid md:grid-cols-3 gap-10 items-center">
                <div className="relative group/upload h-full flex flex-col justify-center">
                   ...
                   <div className="border-2 border-dashed border-slate-700 rounded-3xl p-10 bg-black/40 group-hover/upload:border-cyan-500 transition-all text-center">
                      ...
                      <span className="text-sm text-white font-bold uppercase tracking-[0.2em]">Upload New Asset</span>
                   </div>
                </div>
                   ...
                   <div className="flex-1">
                      <div className="flex justify-between items-start mb-6">
                         <div className="inline-flex items-center gap-2 px-3 py-1 bg-cyan-500/10 border border-cyan-500/20 rounded-full">
                            ...
                            <span className="text-[10px] font-bold text-cyan-400 uppercase tracking-widest">Active Production Unit</span>
                         </div>
                         <div className="flex flex-col items-end gap-2">
                            <span className="text-cyan-400 font-mono font-black text-xl bg-cyan-900/20 px-4 py-1 rounded-lg border border-cyan-500/30 shadow-[0_0_15px_rgba(6,182,212,0.2)]">CREDITS: {credits}</span>
                            <button onClick={() => document.getElementById('pricing').scrollIntoView()} className="text-[9px] font-black text-white uppercase bg-blue-600 hover:bg-blue-500 px-3 py-1.5 rounded transition-colors shadow-lg tracking-widest">+ Add More Credits</button>
                         </div>
                      </div>
                      <h3 className="text-white font-black text-3xl uppercase tracking-tighter mb-4">
                         {pendingFile ? pendingFile.name : "Waiting for Assets"}
                      </h3>
                      <div className="flex gap-6 border-t border-slate-800 pt-4">
                         <div className="flex flex-col">
                            <span className="text-[9px] text-slate-500 uppercase font-black tracking-widest mb-1">System Core</span>
                            <span className="text-xs text-slate-300 font-bold uppercase tracking-tight">AI Vectorization Engine v1.6.2</span>
                         </div>
                         {result && (
                           <div className="flex flex-col border-l border-slate-800 pl-6">
                              <span className="text-[9px] text-slate-500 uppercase font-black tracking-widest mb-1">Job Status</span>
                              <span className="text-xs text-green-400 font-bold uppercase tracking-tight">Success: {result.processing_time}s</span>
                           </div>
                         )}
                      </div>
                   </div>
              ...
              <div className="animate-fade-in p-8 rounded-3xl border-l-[12px] flex items-center justify-between shadow-2xl bg-orange-950/40 border-orange-600">
                 <div className="flex items-center gap-8">
                    ...
                    <div>
                       ...
                       <p className="text-slate-300 text-sm font-bold uppercase tracking-tight opacity-80 mt-1">{qualityReport.message}</p>
                    </div>
                 </div>
                 <div className="flex items-center gap-8">
                      ...
                      <div className="flex flex-col justify-center gap-8">
                         {isProcessing ? (
                            <div className="bg-black/40 rounded-3xl p-10 border border-slate-800 animate-fade-in h-full flex flex-col justify-center">
                               <div className="text-cyan-400 text-3xl font-black mb-10 flex items-center gap-4"><div className="w-4 h-4 bg-cyan-500 rounded-full animate-ping"></div>{processStatus}</div>
                            ...
                            <div className="space-y-8">
                               <div className="bg-slate-950 p-8 rounded-2xl border border-slate-800 shadow-xl">
                                  <h5 className="text-[10px] text-slate-500 uppercase font-black mb-6 tracking-widest border-b border-slate-900 pb-3">Technical Analysis Result</h5>
                                  <div className="grid grid-cols-3 gap-8 font-mono text-xs">
                                     <div><span className="text-slate-600 block mb-2 uppercase text-[9px] font-black">Width</span><span className="text-cyan-400 font-black text-lg">{result.width_mm} mm</span></div>
                                     <div><span className="text-slate-600 block mb-2 uppercase text-[9px] font-black">Height</span><span className="text-cyan-400 font-black text-lg">{result.height_mm} mm</span></div>
                                     <div><span className="text-slate-600 block mb-2 uppercase text-[9px] font-black">Nodes</span><span className="text-white font-black text-lg">{result.node_count || 384}</span></div>
                                  </div>
                               </div>
                               <div className="grid gap-4">
                                  <a href={`${API_URL}${result.files.dxf}`} className="group p-6 bg-slate-950 border border-slate-800 rounded-2xl hover:border-cyan-500 transition-all flex items-center justify-between">
                                  ...
                                  <a href={`${API_URL}${result.files.svg}`} className="group p-6 bg-slate-950 border border-slate-800 rounded-2xl hover:border-blue-500 transition-all flex items-center justify-between">
                                  ...
                                  <a href={`${API_URL}${result.files.zip}`} className="group p-8 bg-gradient-to-br from-cyan-600/30 to-blue-600/30 border border-cyan-500/40 rounded-2xl hover:shadow-[0_0_50px_rgba(6,182,212,0.3)] transition-all flex items-center justify-between border-l-[12px] border-l-cyan-500">
                                     <div className="font-black text-white text-2xl tracking-tighter uppercase italic">Download Production Bundle</div>
                    ...
                    <div className="grid md:grid-cols-2 gap-12 text-[12px] leading-relaxed tracking-wider">
                       <div className="space-y-8">
                          <div>
                             ...
                             <p className="text-slate-300 font-bold uppercase opacity-70">The analysis above indicates the source quality of your upload. If "Warning" is displayed, the input image was identified as low-resolution or blurry. By using these files, you acknowledge that output precision is directly linked to input quality.</p>
                          </div>
                          <div>
                             <h5 className="text-yellow-500 font-black uppercase mb-3 tracking-[0.2em] text-[13px]">2. Check Before Cutting:</h5>
                             <p className="text-slate-300 font-bold uppercase opacity-70">Cuttora uses automated AI tools. While we strive for perfection, errors can occur. ALWAYS verify dimensions, curves, and cut paths in your CAD software (AutoCAD, LightBurn, etc.) BEFORE sending files to your machine.</p>
                          </div>
                       </div>
                       <div className="space-y-8">
                          <div>
                             <h5 className="text-yellow-500 font-black uppercase mb-3 tracking-[0.2em] text-[13px]">3. Liability Limitation:</h5>
                             <p className="text-slate-300 font-bold uppercase opacity-70">Cuttora is NOT responsible for any wasted materials, machine time, or production errors resulting from the use of these files. User assumes full responsibility for the final manufacturing output.</p>
                          </div>
                          <div className="bg-green-500/5 border border-green-500/20 p-8 rounded-2xl shadow-inner">
                             ...
                             <p className="text-green-200 font-bold uppercase opacity-70">If this conversion is not usable or contains errors: DO NOT cut the material. Contact us immediately at support@cuttora.com. We will refund your credits instantly or manually fix the file for you.</p>
                          </div>
                       </div>
                    </div>
                  </div>
