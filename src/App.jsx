      ...
      {apiKey ? (
        <section className="pt-8 pb-24 px-6 min-h-screen relative z-10 bg-[#020617] cyber-grid">
          <div className="max-w-6xl mx-auto space-y-8">
            ...
            {/* 3. İSTASYON: ÜRETİM MERKEZİ VE TAM DOSYA SETİ */}
			<div className={`transition-all duration-700 ${result || isProcessing ? 'opacity-100' : 'opacity-20 pointer-events-none'}`}>
			  <div className="bg-slate-900 border border-slate-800 rounded-[2.5rem] p-12 shadow-2xl">
				<div className="grid lg:grid-cols-2 gap-16">
				  
				  {/* SOL: AYDINLATILMIŞ SVG PREVIEW (Koyu Ekran Sorunu Çözüldü) */}
				  <div className="space-y-6">
					<div className="bg-white/[0.03] rounded-[3rem] border border-slate-800 p-10 relative aspect-square flex items-center justify-center shadow-inner overflow-hidden">
					  <div className="absolute inset-0 bg-gradient-to-tr from-cyan-500/5 to-blue-500/5 pointer-events-none"></div>
					  {isProcessing ? (
						<div className="text-center">
						  <div className="text-cyan-400 text-3xl font-black uppercase animate-pulse mb-4">FABRICATING...</div>
						...
						<img 
						  ...
						  className="max-w-full max-h-full drop-shadow-[0_0_50px_rgba(255,255,255,0.15)] brightness-125 contrast-125" 
						  alt="High-Visibility Preview" 
						/>
					  )}
					</div>
				  </div>

				  {/* SAĞ: TAM DONANIMLI İNDİRME MERKEZİ (DXF, EPS, SVG, ZIP) */}
				  <div className="flex flex-col justify-center gap-6">
					{result && (
					  <>
						<div className="bg-slate-950 p-8 rounded-3xl border border-slate-800 mb-2 shadow-xl">
						  <h5 className="text-[11px] text-slate-500 uppercase font-black mb-6 tracking-[0.4em] border-b border-slate-900 pb-3">Technical Output Data</h5>
						...
						<div className="grid grid-cols-2 gap-4">
						  {/* DXF BUTONU - ARTIK AKTİF */}
						  <a href={`${API_URL}${result.files.dxf}`} download className="group p-5 bg-slate-950 border border-slate-800 rounded-2xl hover:border-cyan-500 transition-all flex items-center justify-between">
							<span className="font-black text-white text-[12px] uppercase">Industrial DXF</span>
						  ...
						  <a href={`${API_URL}${result.files.svg}`} download className="group p-5 bg-slate-950 border border-slate-800 rounded-2xl hover:border-blue-500 transition-all flex items-center justify-between">
							<span className="font-black text-white text-[12px] uppercase">Master SVG</span>
							<Download className="w-5 h-5 text-slate-600 group-hover:text-blue-400" />
						  </a>
						  {/* EPS BUTONU (YENİ EKLEDİM!) */}
						  <a href={`${API_URL}${result.files.eps || result.files.svg}`} download className="group p-5 bg-slate-950 border border-slate-800 rounded-2xl hover:border-purple-500 transition-all flex items-center justify-between">
							<span className="font-black text-white text-[12px] uppercase">Print EPS</span>
							<Download className="w-5 h-5 text-slate-600 group-hover:text-purple-400" />
						  </a>
						  {/* METADATA/INFO BUTONU */}
						  <div className="p-5 bg-slate-950/50 border border-slate-800 rounded-2xl flex items-center justify-between opacity-50">
							<span className="font-black text-slate-500 text-[12px] uppercase">AI Report PDF</span>
							<Lock className="w-4 h-4 text-slate-700" />
						  </div>
						</div>

						{/* FULL PRODUCTION BUNDLE (TÜM PAKET) */}
						<a href={`${API_URL}${result.files.zip}`} download className="group p-8 bg-gradient-to-br from-cyan-600/30 to-blue-600/30 border border-cyan-500/40 rounded-[2rem] hover:shadow-[0_0_60px_rgba(6,182,212,0.3)] transition-all flex items-center justify-between border-l-[16px] border-l-cyan-500 shadow-2xl mt-4">
						  <div>
							<div className="font-black text-white text-3xl tracking-tighter uppercase italic">Download Full Bundle</div>
							<div className="text-[11px] text-cyan-300 font-bold uppercase mt-2 tracking-widest">DXF, SVG, EPS, PDF & README ALL-IN-ONE</div>
						  </div>
